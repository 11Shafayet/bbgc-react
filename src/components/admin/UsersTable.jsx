...
const UsersTable = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['user'],
    queryFn: async () => {
      const res = await axios.get(`https://bbgc-backend.vercel.app/user/${user?._id}`);
      return res.data;
    },
    enabled: !!user,
  });
  ...
  const handleDelete = async (userId) => {
    try {
      const response = await axios.delete(
        `https://bbgc-backend.vercel.app/user/${userId}`
      );
  ...
  const handleUpdate = async (e, userId, index) => {
    ...
    try {
      ...
      const response = await axios.patch(`https://bbgc-backend.vercel.app/user`, {
        userId,
        updatedRole,
      });
      if (response) {
